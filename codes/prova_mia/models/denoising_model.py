...
class DenoisingModel(BaseModel):
    ...
    def feed_data(self, state, LQ, GT=None):
        self.state_t = state.to(self.device)    # noisy_state
        self.state_T = LQ.to(self.device)  # LQ
        if GT is not None:
    ...
    def optimize_parameters(self, step, timesteps, sde=None):
        ...
        # Get noise and score
        noise = sde.noise_fn(self.state_t, timesteps.squeeze())
    ...
    def test(self, sde=None, save_states=False):
        ...
        self.model.eval()
        with torch.no_grad():
            self.output = sde.reverse_sde(self.state_t, save_states=save_states)
    ...
    def get_current_visuals(self, need_GT=True):
        ...
        out_dict["Input"] = self.state_t.detach()[0].float().cpu()
