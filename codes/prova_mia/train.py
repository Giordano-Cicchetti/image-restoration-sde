...
def main():
    ...
    sde = util.GeneralDiffusion(max_sigma=opt["sde"]["max_sigma"], T=opt["sde"]["T"], schedule=opt["sde"]["schedule"], eps=opt["sde"]["eps"], device=device)
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            timesteps, states = sde.generate_random_states(x0=GT, xT=LQ)

            model.feed_data(states, LQ, GT) # xt, mu, x0
            loss = model.optimize_parameters(current_step, timesteps, sde)
            ...
            if current_step % opt["logger"]["print_freq"] == 0:
                ...
                message = "<epoch:{:3d}, iter:{:8,d}, lr:{:.3e}, loss:{:.5e}> ".format(
                    epoch, current_step, model.get_current_learning_rate(),loss
                )
                print(message)
